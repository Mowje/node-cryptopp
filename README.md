...
* if keyType is ECDSA or ECIES
	...
	* publicKeyX.length : length of the x coordinate of the public point (2 bytes)
	* publicKeyX : x coordinate of the public point
	* publicKeyY.length : length of the y coordinate of the public point (2 bytes)
	* publicKeyY : y coordinate of the public point
	* privateKey.length : length of the private key (2 bytes)
	* privateKey
* if keyType is RSA
	* modulus.length : length of the RSA modulus (2 bytes)
	* modulus : RSA modulus
	* publicExponent.length : length of the public exponent (2 bytes)
	* publicExponent : RSA public exponent (or public key)
	* privateExponent.length : length of the private exponent (2 bytes)
	* privateExponent : RSA private exponent (or private key)
* if keyType is DSA
	* primeField.length : length of the prime field used by the DSA key pair (2 bytes)
	* primeField
	* divider.length : length of the divider (2 bytes)
	* divider
	* base.length : length of the base (2 bytes)
	* base : DSA base
	* publicElement.length : length of the DSA public key (2 bytes)
	* publicElement : DSA public key
	* privateExponent.length : length of the DSA private exponent (2 bytes)
	* privateExponent : DSA private exponent (or private key)
* if keyType is ECDH
	* curveID : a byte, corresponding to the curve used
	* publicKey.length : length of the ECDH public key (2 bytes)
	* publicKey : ECDH public key
	* privateKey.length : length of the ECDH private key (2 bytes)
